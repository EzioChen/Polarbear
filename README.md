...
- **邮件配置全局化**: 邮件配置从项目级改为 VS Code 全局配置，跨项目共享配置
  - 配置存储在 VS Code 用户设置中 (`polarbear.email.*`)
  - 密码安全存储在 VS Code Secret Storage 中
  - 支持通过 VS Code 设置界面直接编辑
- **邮件发送修复**: 修复邮件编辑器发送按钮无响应的问题
  - 修复 Vue 响应式对象序列化导致的 `DataCloneError`
  - 添加详细的调试日志，便于排查问题
  - 添加连接验证超时保护，防止连接卡住
- **邮件预编辑修复**: 修复发布时间线中邮件预编辑无法加载默认收件人的问题
  - 改为从 VS Code 全局配置读取收件人信息
- **增强日志功能**: 为邮件发送流程添加详细日志
  - 前端控制台日志（按 F12 查看）
  - 后端 PolarBear Email 输出频道日志
